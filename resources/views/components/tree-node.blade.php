{{--
    ...
    - $canModify: Can user create/edit tree nodes
    - $icon: The icon to display for node
--}}
        ...
        <div class="fi-ta-cell p-0">
            ...
            {{-- Drag Handle --}}
                <button
                    ...
                    @if ($canModify)
                        data-drag-handle
                        title="{{ __('filament-treeview::tree.node.drag_to_reorder') }}"
                    @else
                        disabled
                    @endif
                    @class(["relative flex-shrink-0 text-gray-400 hover:text-gray-600 dark:text-gray-500 dark:hover:text-gray-400 transition-opacity", 
                        "filament-tree-drag-handle" => $canModify])                    
                >
                    <span class="absolute -inset-2"></span>
                    <x-dynamic-component :component="$icon" class="text-gray-600 dark:text-gray-400 w-5 h-5"/>
                </button>
                ...
                <div class="filament-tree-toggle-container">
                    @if ($tree->isCollapsible() && $hasChildren)
                        <button
                            ...
                            title="{{ __('filament-treeview::tree.node.toggle') }}"
