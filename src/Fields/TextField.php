...
class TextField extends Field
{
    ...
    public function url(Closure $url, string $linkTarget = '_blank'): static
    {
        $this->url = $url;
    ...
    public function render(Model|array $record): string
    {
        ...
        $url = '';
        if ($this->url) {
            $url = (string) ($this->url)($state, $record);
        }

        if ($url !== '') {
            $el = '<a href="'. $url. '" target="'. $this->linkTarget . '">'. e($formatted). '</a>';
        } else {
            $el = e($formatted);
        }

        if ($classString || $shouldDim) {
            return'<span class="'. $classString. '"'. $style.'>'. $el. '</span>';           
        }
